# 📊 Relatório Individual - EllipticEnvelope

**Modo de Execução**: COMPLETO  
**Data de Geração**: 2025-10-20 00:23:10  
**Total de Configurações**: 15  
**Total de Execuções**: 45

## 🎯 Resumo Executivo

### Melhor Configuração (param_id: 14)
- **F1-Score Médio**: 0.9919 ± 0.0000
- **Accuracy Média**: 0.9842 ± 0.0000
- **Precision Média**: 0.9904 ± 0.0000
- **Recall Médio**: 0.9934 ± 0.0000
- **Tempo de Treinamento Médio**: 14.80s ± 1.16s
- **Execuções**: 3

### Performance Geral (todas as configurações)
- **F1-Score Médio**: 0.9902 ± 0.0014
- **Accuracy Média**: 0.9811 ± 0.0026
- **Precision Média**: 0.9956 ± 0.0028
- **Recall Médio**: 0.9850 ± 0.0054

### Métricas de Qualidade
- **Estabilidade entre Configurações (Desvio F1)**: 0.0014 🟢 Excelente
- **Eficiência Média**: 0.0679 F1/segundo
- **Tempo Médio**: 14.58s ± 0.33s

## 📈 Análise Detalhada

### Distribuição das Métricas (por configuração)

#### Accuracy
- **Mínimo**: 0.9761
- **Q1**: 0.9797
- **Mediana**: 0.9820
- **Q3**: 0.9831
- **Máximo**: 0.9842
- **IQR**: 0.0034

#### Balanced Accuracy
- **Mínimo**: 0.7960
- **Q1**: 0.8633
- **Mediana**: 0.9023
- **Q3**: 0.9470
- **Máximo**: 0.9829
- **IQR**: 0.0837

#### Precision
- **Mínimo**: 0.9904
- **Q1**: 0.9937
- **Mediana**: 0.9956
- **Q3**: 0.9979
- **Máximo**: 0.9998
- **IQR**: 0.0042

#### Recall
- **Mínimo**: 0.9758
- **Q1**: 0.9813
- **Mediana**: 0.9859
- **Q3**: 0.9889
- **Máximo**: 0.9934
- **IQR**: 0.0077

#### F1 Score
- **Mínimo**: 0.9876
- **Q1**: 0.9895
- **Mediana**: 0.9907
- **Q3**: 0.9913
- **Máximo**: 0.9919
- **IQR**: 0.0018

### Análise de Parâmetros


#### contamination
- **Melhor valor**: 0.4 (F1: 0.9919)
- **Variação observada**: 15 valores diferentes
- **Impacto no F1**: 0.0043

## 🎯 Recomendações

### Pontos Fortes
- ✅ **Excelente performance geral** (F1 > 0.8)
- ✅ **Alta estabilidade** entre execuções
- ✅ **Boa eficiência computacional**

### Áreas de Melhoria

## 📊 Arquivos Gerados

### Gráficos
- `plots/performance_evolution.png` - Evolução das métricas
- `plots/parameter_impact.png` - Impacto dos parâmetros
- `plots/confusion_matrix_analysis.png` - Análise da matriz de confusão
- `plots/metrics_distribution.png` - Distribuição das métricas
- `plots/execution_time_analysis.png` - Análise de tempo

### Tabelas
- `tables/descriptive_statistics.csv` - Estatísticas descritivas
- `tables/detailed_results.csv` - Resultados detalhados
- `tables/execution_ranking.csv` - Ranking por execução

---
*Relatório gerado automaticamente pelo sistema de análise individual*
